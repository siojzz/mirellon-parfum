...
@echo off
setlocal
title Pembersih & Pengatur Folder Downloads
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "Invoke-Expression ([System.IO.File]::ReadAllText('%~f0'))"
pause
goto :eof
#>
...
# Coba ambil dari Windows Shell KnownFolder Registry
try {
...
# Kategori folder yang tidak boleh dipindahkan / diproses
$categoryFolderNames = @($categories.Keys) + @("Lain-lain")

# File sementara/download yang belum selesai diabaikan
...
function Run-Cleaner {
    ...
    # Ambil semua file langsung di root Downloads
    $dirInfo = New-Object System.IO.DirectoryInfo($downloadsPath)
    $allFiles = $dirInfo.GetFiles()

    $validFiles = @()
    foreach ($f in $allFiles) {
        # Lewati file tersembunyi/sistem atau file yang diabaikan
    ...
    foreach ($file in $validFiles) {
        ...
        if ($dryRun) {
            ...
            # Pastikan folder kategori tujuan sudah dibuat
            ...
            # Dapatkan path yang aman dari penimpaan nama
...
switch ($pilihan.Trim()) {
    "1" {
        Run-Cleaner -dryRun $false
    }
    "2" {
        Run-Cleaner -dryRun $true
    }
    "3" {
        Invoke-Item -LiteralPath $downloadsPath
    }
    "4" {
        Write-Host "`nKeluar dari program. Terima kasih!" -ForegroundColor Yellow
    }
    default {
        Write-Host "`nPilihan tidak dikenal. Silakan jalankan ulang dan pilih angka 1-4." -ForegroundColor Red
    }
}
